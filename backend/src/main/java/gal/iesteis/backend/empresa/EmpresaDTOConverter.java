...
public class EmpresaDTOConverter {
    ...
    public Empresa DTOCreateAEmpresa(EmpresaDTOCreate dto, UserDetailsImpl userDetails) {
        ...
        Especialidad especialidad = isAdmin ? especialidadService.obtenerPorId(dto.getEspecialidad())
                : (tutorCentroRepository.findById(userDetails.getTutorCentroId())
                        .orElseThrow(() -> new TutorCentroNotFoundException(userDetails.getTutorCentroId())).getCurso()
                        .getEspecialidad());
