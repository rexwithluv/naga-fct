...
public class SkillService {
    ...
    @Autowired
    private SkillDTOConverter dtoConverter;
    ...
    public List<SkillDTO> obtenerTodas(UserDetailsImpl userDetails) {
        if (AuthUtils.isAdmin(userDetails)) {
            return repository.findAll().stream().map(skill -> dtoConverter.skillADtoResponse(skill)).toList();
        }

        TutorCentro tutorUsuario = tutorCentroService.obtenerTutorCentroPorid(userDetails.getTutorCentroId());
        ...
        return repository.findByEspecialidad(especialidadUsuario).stream()
                .map(skill -> dtoConverter.skillADtoResponse(skill)).toList();
